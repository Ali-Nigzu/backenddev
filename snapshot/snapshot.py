...
def _run_attempt(resources, sql_connection, organisation_id, stats):
    ...
    state_size = 0 if org_candidate is None else len(json.dumps(org_candidate.state))
    print(json.dumps({"organisation_id": organisation_id, "changed_sites": len(site_candidates),
        "org_classification": org_classification, "bq_queries": stats.bq_queries,
        "events": stats.events_normalized, "provisional_events": 0 if org_candidate is None else len(org_candidate.state["provisional_events"]),
        "state_bytes": state_size}, sort_keys=True))
    return True
