...
def detection(frame_id, detection_id, x, y=50.0, confidence=0.9, width=20.0, height=40.0):
    return {
...
def batch(points, confidence=0.9):
    ...
    for index, (timestamp, x) in enumerate(points):
        frame_id = f"frame-{index:02d}"
        frames.append(
            frame(index, timestamp, [detection(frame_id, f"det-{index}", x, confidence=confidence)])
        )
...
class TrackTests(unittest.TestCase):
    ...
    def test_determinism_and_canonical_frame_order(self):
        ...
        self.assertEqual([point["timestamp"] for point in first["tracks"][0]["path"]], [0.0, 0.3, 0.7])
    ...
    def test_later_birth_requires_normal_activation_hits(self):
        ...
        for index, x in enumerate((20.0, 22.0), start=10):
            frame_id = f"frame-{index:02d}"
            frames.append(frame(index, index * 0.1, [detection(frame_id, f"det-{index}", x)]))
    ...
    def test_low_confidence_recovers_active_track(self):
        ...
        frames.append(frame(2, 0.6, [detection("frame-02", "det-2", 16.0, confidence=0.3)]))
    ...
    def test_short_disappearance_recovers_same_track(self):
        frames = batch([(0.0, 10.0), (0.3, 13.0)])["detections"]
        frames.extend([frame(2, 0.8), frame(3, 1.3, [detection("frame-03", "det-3", 23.0)])])
        result = Track()({"detections": frames})
    ...
    def test_crossing_pedestrians_keep_trajectory_direction(self):
        ...
        paths = [[point["centre"]["x"] for point in track["path"]] for track in result["tracks"]]
        self.assertTrue(all(later > earlier for earlier, later in zip(paths[0], paths[0][1:])))
        self.assertTrue(all(later < earlier for earlier, later in zip(paths[1], paths[1][1:])))

    def test_irregular_intervals_and_zero_dt(self):
        result = Track()(batch([(0.0, 10.0), (0.31, 13.1), (0.70, 17.0), (1.08, 20.8), (1.62, 26.2)]))
        self.assertEqual(len(result["tracks"]), 1)
    ...
    def test_best_crop_uses_earliest_highest_confidence(self):
        ...
        for index, confidence in enumerate((0.8, 0.9, 0.9)):
            frame_id = f"frame-{index:02d}"
            frames.append(
                frame(index, index * 0.3, [detection(frame_id, f"det-{index}", 10 + index * 2, confidence=confidence)])
            )
    ...
    def test_event_accepts_track_batch_without_adapter(self):
        ...
        event_batch = Event()(result, {"point_a": {"x": 0.0, "y": 50.0}, "point_b": {"x": 100.0, "y": 50.0}})
    ...
    def test_invalid_timestamps_fail_clearly(self):
        for timestamp in (math.nan, math.inf, -math.inf):
            with self.subTest(timestamp=timestamp), self.assertRaisesRegex(ValueError, "finite"):
    ...
    def test_math_stays_finite_positive_and_gated_pairs_are_rejected(self):
        ...
        _residual, innovation_covariance, distance = track_module._innovation(internal, source)
        ...
        matches, unmatched_tracks, unmatched_detections = track_module._associate([internal], [far], 1.0)
